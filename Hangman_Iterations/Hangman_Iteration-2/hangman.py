...
def show_answer(answer, selected_word, missing_letter_index):
    """
    message that is printed right at the end
    but the message printed depends on your results
    """
...
def ask_file_name():
    """
    user is asked which file they will use
    if input is left empty, a default file will be used
    """
    demo = input("Words file? [leave empty to use short_words.txt] :")
    if demo == '':
